    ...
    init {
        viewModelScope.launch {
            streamCinemaRepository.mediaFilter().collect { resource ->
                _uiState.update {
                    when(resource) {
                        is Resource.Error -> it.copy(isLoading = false, errorToast = resource.message ?: "Something went wrong")
                        is Resource.Loading -> it.copy(isLoading = true)
                        is Resource.Success -> it.copy(isLoading = false, errorToast = null, items = resource.data.orEmpty())
                    }
                }
            }
        }
